...
finalProj$BureauEnergy$data$Imp_EnergySourc$日期<- factor(finalProj$BureauEnergy$data$Imp_EnergySource$Source, levels=c("生質能及廢棄物", "核能", "天然氣", "煤及煤產品","原油及石油產品"))
```

```{r}
areaColor= c("#BEBEBE","#D0D0D0", "#E0E0E0", "#6C6C6C", "#4F4F4F", "black")
...
ggplot()+
  ...
  geom_text(
    mapping = aes(
      ...
      label = c("Taiwan","World", "EU", "USA", "China")
    ),
...
ggplot()+
  ...
  geom_label(
    data=labels,
    mapping=aes(label = source, x= 3.5, y = Mid),
    size = 2,
    position="identity"
  )+
  xlim(0, 3.5)+
  labs(title = "台灣發電來源",
       caption = "資料來源: 經濟部能源局",
       x = NULL, y = NULL) + 
  theme(axis.ticks=element_blank(), axis.text=element_blank(), axis.title=element_blank())+theme_void()+
  coord_polar(theta = "y") 
```
